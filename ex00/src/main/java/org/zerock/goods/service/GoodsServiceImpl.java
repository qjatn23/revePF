...
public class GoodsServiceImpl implements GoodsService {
	...
	@Override
	public List<GoodsVO> getGoodsListByIds(List<Long> goodsIds) {
	    if (goodsIds == null || goodsIds.isEmpty()) {
	        return new ArrayList<>();
	    }
	    return mapper.selectGoodsByIds(goodsIds);
	}
    ...
    public Integer delete(Long goods_no) {
        log.info("상품 삭제 처리 - 상품 번호: " + goods_no);
        return mapper.delete(goods_no);
    }
	...
	public List<GoodsVO> list(PageObject pageObject, GoodsSearchVO goodsSearchVO) {
	    // 상품의 총 개수를 가져오기 전에 정렬 기준에 맞는 쿼리를 설정합니다.

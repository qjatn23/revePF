	...
	<div class="container">
		<div class="card">
			...
			<form action="update.do" method="post" enctype="multipart/form-data">
				...
				<%-- 			<input type="hidden" name="brandsSearchVO.brands_cate_code1" value="${brandsSearchVO.brands_cate_code1 }">  			 --%>
				<%-- 			<input type="hidden" name="brandsSearchVO.brands_cate_code2" value="${brandsSearchVO.brands_cate_code2 }">  			 --%>
				<%-- 			<input type="hidden" name="brandsSearchVO.brands_name" value="${brandsSearchVO.brands_name }">  			 --%>
				<%-- 			<input type="hidden" name="brandsSearchVO.min_price" value="$brandsSearchVO.min_price }">  			 --%>
				<%-- 			<input type="hidden" name="brandsSearchVO.max_price" value="${brandsSearchVO.max_price }">  			 --%>
				<input type="hidden" name="brands_price_no"
					value="${brandsVO.brands_price_no }">
				<div class="card-body">
					<div class="form-group">
						<label for="brands_no">상품번호</label> <input class="form-control"
							id="brands_no" name="brands_no" readonly
							value="${brandsVO.brands_no }">
					</div>
					<!-- 대분류, 중분류는 java 구현후 작성할 예정 -->
					<div class="form-group">
						<label for="brands_cate_code1">대분류</label> <select
							class="form-control" id="brands_cate_code1"
							name="brands_cate_code1">
							<c:forEach items="${listBig }" var="listVO">
								<option value="${listVO.brands_cate_code1 }"
									${(brandsVO.brands_cate_code1 == listVO.brands_cate_code1)?'selected':''}>
									${listVO.brands_cate_name }</option>
							</c:forEach>
						</select>
					</div>
					<div class="form-group">
						<label for="brands_cate_code2">중분류</label> <select
							class="form-control" id="brands_cate_code2"
							name="brands_cate_code2">
							<c:forEach items="${listMid }" var="listVO">
								<option value="${listVO.brands_cate_code2 }"
									${(brandsVO.brands_cate_code2 == listVO.brands_cate_code2)?'selected':''}>
									${listVO.brands_cate_name }</option>
							</c:forEach>
						</select>
					</div>
										<div class="form-group">
						<label for="brands_cate_name">브랜드</label> <input class="form-control"
							id="brands_cate_name" name="brands_cate_name" required
							value="${brandsVO.brands_cate_name }">
					</div>
					<div class="form-group">
						<label for="brands_name">상품명</label> <input class="form-control"
							id="brands_name" name="brands_name" required
							value="${brandsVO.brands_name }">
					</div>
					<div class="form-group">

						<label for="imageMain">대표이미지</label> <img alt=""
							src="${brandsVO.brands_image_name }"
							style="width: 100px; height: 100px;">
					</div>
					<div class="form-group">
						<label for="content">상품설명</label>
						<textarea rows="10" class="form-control" id="content"
							name="content">${brandsVO.content }</textarea>
					</div>
					<div class="form-group">
						<label for="company">제조사</label> <input class="form-control"
							id="company" name="company" required value="${brandsVO.company }">
					</div>
					<div class="form-group">
						<label for="product_date">생산일</label> <input
							class="form-control datepicker" readonly id="product_date"
							name="product_date"
							value="<fmt:formatDate value='${brandsVO.product_date }' pattern='yyyy-MM-dd' />">
					</div>
					<div class="form-group">
						<label for="price">정가</label> <input class="form-control"
							id="price" name="price" required value="${brandsVO.price }">
					</div>
					<div class="form-group">
						<label for="discount">할인금액</label> <input class="form-control"
							id="discount" name="discount" value=${brandsVO.discount }>
					</div>
					<div class="form-group">
						<label for="discount_rate">할인율</label> <input class="form-control"
							id="discount_rate" name="discount_rate"
							value="${brandsVO.discount_rate }">
					</div>
					<div class="form-group">
						<label for="saved_rate">적립율</label> <input class="form-control"
							id="saved_rate" name="saved_rate" value="${brandsVO.saved_rate }">
					</div>
					<div class="form-group">
						<label for="delivery_charge">배송료</label> <input
							class="form-control" id="delivery_charge" name="delivery_charge"
							value="${brandsVO.delivery_charge }">
					</div>
					<div class="form-group">
						<label for="sale_start_date">판매시작일</label> <input
							class="form-control datepicker" readonly id="sale_start_date"
							name="sale_start_date"
							value="<fmt:formatDate value='${brandsVO.sale_start_date }' pattern='yyyy-MM-dd' />">
					</div>
					<div class="form-group">
						<label for="sale_end_date">판매종료일</label> <input
							class="form-control datepicker" readonly id="sale_end_date"
							name="sale_end_date"
							value="<fmt:formatDate value='${brandsVO.sale_end_date }' pattern='yyyy-MM-dd' />">
					</div>
						...
						<fieldset class="border p-4 colorForm">
							...
							<c:if test="${!empty colorList }">
								<c:forEach items="${colorList}" var="colorVO" varStatus="status">
									<div class="input-group mb-3">
										<input type="text" class="form-control" name="color_names"
											value=${colorVO.brands_color_name }>
					...
					<fieldset class="border p-4 imageForm">
						...
						<c:if test="${!empty imageList }">
							<c:forEach items="${imageList}" var="imageVO" varStatus="status">
								<div class="input-group mb-3">
									<img src="${imageVO.brands_image_name }"
...
$(function(){
	...
	// 대분류 리스트 변경
	$("#brands_cate_code1").change(function(){
		//alert("대분류 리스트 변경");
		...
		$.ajax({
			type: "get",
			url: "/brands/getbrandsCategory.do?brands_cate_code1=" + brands_cate_code1,
			...
			success: function(result, status, xhr) {
				...
				result.forEach(function(item){
					console.log(item.brands_cate_name);
					str += '<option value="' + item.brands_cate_code2 + '">';
					str += item.brands_cate_name + '</option>\n';
				});
				
				console.log(str);
				
				$("#brands_cate_code2").html(str);
			},

...
<body>
	<div class="container" style="padding-top: 200px;">
		<div class="card" style="border: none;">
				...
				<div class="row">
					<div class="col-md-6 description-box">
						...
						<div id="smallImageDiv">
							<img src="${vo.brands_image_name }" class="img-thumbnail">
							<c:if test="${!empty imageList }">
								<c:forEach items="${imageList}" var="imageVO">
									<img src="${imageVO.brands_image_name }" class="img-thumbnail">
								</c:forEach>
							</c:if>
						</div>
					</div>
					<div class="col-md-6" id="goodsDetailDiv">
						<h5 class="product_no">${vo.brands_no }</h5>
						<h2 class="product_brand">${vo.brands_cate_name }</h2>
						<h3 class="product_title">${vo.brands_name }</h3>
						...
						<div class="form-group size-color-container">
							...
							<div class="size-container">
								<c:forEach items="${sizeList}" var="sizeVO">
									<label> <input type="radio" name="brands_size_name"
										value="${sizeVO.brands_size_name}" class="size-radio" /> <span
										class="size-label">${sizeVO.brands_size_name}</span>
									</label>
								</c:forEach>
							</div>
							<div class="color-container">
								<c:forEach items="${colorList}" var="colorVO">
									...
									<label class="color-label"
										style="background-color: ${colorVO.brands_color_name};">
										<input type="radio" name="brands_color_name"
										value="${colorVO.brands_color_name}" class="color-radio"
										id="color_${colorVO.brands_color_name}" />
									</label>
								</c:forEach>
							</div>
						</div>
						<div class="product_view_btn">
							<form action="/cart/add" method="POST" class="cart-form">
								<input type="hidden" name="id" value="${login.id}" /> <input
									type="hidden" name="goods_no" value="${vo.brands_no}" /> <input
									type="hidden" name="goods_name" value="${vo.brands_name}" /> <input
									type="hidden" name="image_name"
									value="${vo.brands_image_name}" /> <input type="hidden"
			...
			<div class="card-footer">
				...
				<form action="${pageContext.request.contextPath}/brands/delete.do"
					method="post">
					<input type="hidden" name="brands_no" value="${vo.brands_no}" />
	...
	$(function() {
	    ...
	    $("#updateBtn")
		    .click(
			    function() {
				location = "updateForm.do?brands_no=${vo.brands_no}&page=${param.page}"
					+ "&perPageNum=${param.perPageNum}"
					+ "&${brandsSearchVO.searchQuery}";
			    });
